...
import { fetchCartsAll, fetchCartUser } from "../../redux/cart.reducer";

interface OriginData extends IBill {
  recordKey: string;
}
...
const ListBillPage = () => {
  ...
  const fetchCarts = async (_id: string) => {
    try {
      const data = await dispatch(fetchCartsAll(_id)).unwrap();
